...
public class DeathSignHandler {
    ...
    public static DeathSign getDeathSignAtLoc(Location loc) {
        ...
        for (String uuid : Conf.storage.getConfig().getConfigurationSection("player").getKeys(false)) {
            if (Conf.storage.getConfig().contains("player." + uuid + ".deaths"))
                for (String id : Conf.storage.getConfig().getConfigurationSection("player." + uuid + ".deaths").getKeys(false)) {
    ...
    public static void updateSigns(UUID player) {
        ...
        if (Conf.storage.getConfig().contains(path))
            for (String id : Conf.storage.getConfig().getConfigurationSection(path).getKeys(false)) {
    ...
    public static DeathSign createDeathSign(ItemStack[] items, UUID owner, Location location, String deathMessage, boolean demo) {
        int id = 1;
        ...
        if (demo) {
            Conf.storage.getConfig().set("player." + owner + ".deaths." + id + ".status", "DEMO");
        } else
    ...
    public static void breakBrokenSign(Player breaker, Block sign) {
        ...
        if (Conf.deathsign_reward_enabled && Econ.set_up) {
            ...
            if (!success) Main.plugin.getLogger().info("Reward transaction failed! Player didn't receive reward!");
                ...
                breaker.sendMessage(Main.plugin.getConfig().getString("econ.deathsign.reward_message").replace("{reward}", Conf.deathsign_reward + "").replace("&", "§"));
        }
                ...
                for (int x = -(currentDistance); x <= currentDistance; x++) {
                    for (int y = 0; y <= (currentDistance * 2); y++) {
                        ...
                        if ((nearbyBlock.getType().equals(Material.AIR)
                            ...
                            if (DeathSignHandler.getDeathSignAtLoc(nearbyBlock.getRelative(0, 1, 0).getLocation()) != null)
                                continue;
                            if (DeathSignHandler.getDeathSignAtLoc(nearbyBlock.getLocation()) != null) continue;
                            if (Events.deathsignQueue.contains(nearbyBlock.getLocation()) || Events.deathsignQueue.contains(nearbyBlock.getRelative(0, 1, 0).getLocation()))
                                continue;//TODO not even fuckin used
